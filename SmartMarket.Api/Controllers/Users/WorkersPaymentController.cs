...
namespace SmartMarket.Api.Controllers.Users
{
    public class WorkersPaymentController : BaseController
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync([FromRoute(Name = "id")] long id, [FromBody] WorkersPaymentForUpdateDto dto)
            => Ok(new Response
            {
                Code = 200,
                Message = "Success",
                Data = await _workersPaymentService.ModifyAsync(id, dto)
            });
